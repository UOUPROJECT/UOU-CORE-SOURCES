...
static bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("UOUMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("UOUMiner: ProcessNewBlock, block not accepted");
...
void static BitcoinMiner(CWallet *pwallet, uint32_t minerI, uint32_t minerN, int nThreads)
{
        ...
        while (true) {
            ...
            if(ma!=""){
            	if(validateAddress(ma)) {
                    LogPrintf("UOUMiner: Mining to User supplied address %s\n",ma);
                    pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithAddress(ma));
                }else{
                    LogPrintf("UOUMiner: WARNING! User supplied address is invalid, defaulting to keypool address.\n");
                    pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithKey(reservekey));
                }
            }else{
                LogPrintf("UOUMiner: Mining to keypool address.\n");
                pblocktemplate= auto_ptr<CBlockTemplate>(CreateNewBlockWithKey(reservekey));
            }
            if (!pblocktemplate.get())
            {
                LogPrintf("UOUMiner: ERROR! Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running UOUMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("UOUMiner runtime error: %s\n", e.what());
        //aligned_free(scratchpad);
        fGenerate = false;
        return;
    }
