...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(BitcoinUnits::UOU)
    {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->ownedTable->setRowCount(3);

    // actually update labels
    int nDisplayUnit = BitcoinUnits::UOU;
    // Disable sorting outside the for loop
    ui->ownedTable->setSortingEnabled(false);

    if(sort_flag){
     ui->ownedTable->setSortingEnabled(true);
    }
}
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("UOU")
    updateDisplayUnit();
}
