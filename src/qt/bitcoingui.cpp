    ...
    miningOnAction(0)
{
    ...
    QString windowTitle = tr("UOU Core") + " - ";
#ifdef ENABLE_WALLET
    ...
    {
        progressBar->setStyleSheet("QProgressBar { background-color: #f75026; border: 1px solid grey; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #FF8000, stop: 1 orange); border-radius: 7px; margin: 0px; }");
    }
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a UOU address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and UOU: URIs)"));
    ...
    pipeManAction = new QAction(SingleColorIcon(":/icons/pipe"), tr("&Pipe"), this);
    pipeManAction->setStatusTip(tr("Will be available starting March 1, 2019"));
    pipeManAction->setToolTip(pipeManAction->statusTip());
    pipeManAction->setCheckable(false);
    pipeManAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    pipeManAction->setEnabled(false);
    tabGroup->addAction(pipeManAction);
    ...
    connect(pipeManAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    //connect(pipeManAction, SIGNAL(triggered()), this, SLOT(gotoPipeManPage()));
    ...
    aboutAction = new QAction(TextColorIcon(":/icons/about"), tr("&About UOU Core"), this);
    aboutAction->setStatusTip(tr("Show information about UOU Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for UOU Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your UOU addresses to prove you own them"));
    verifyMessageAction = new QAction(TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified UOU addresses"));
    ...
    openAction->setStatusTip(tr("Open a UOU: URI or payment request"));

    showHelpMessageAction = new QAction(TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the UOU Core help message to get a list with possible UOU command-line options"));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        toolbar->addAction(pipeManAction);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    pipeManAction->setEnabled(enabled);
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("UOU Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::gotoPipeManPage(QString addr)
{
    pipeManAction->setChecked(true);
    if (walletFrame) walletFrame->gotoPipeManPage(addr);
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to UOU network", "", count));
}
...
{
    QString strTitle = tr("UOU"); // default title
